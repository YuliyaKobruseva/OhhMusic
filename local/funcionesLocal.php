...
function imagenConcierto($nombre_img, $tipo, $tamano, $user, $tmp) {
    ...
    if (($nombre_img == !NULL) && ($tamano <= 1000000)) {        
        if (($tipo == "image/gif") || ($tipo == "image/jpeg") || ($tipo == "image/jpg") || ($tipo == "image/png")) {            
            ...
            if (move_uploaded_file($tmp, $ruta)) {
                ...
                array_push($resultado, 0, "Error al subir imagen");
                return $resultado;
            }
        } else {            
            array_push($resultado, 0, "Formato de imagen incorrecto");
            return $resultado;
        }
    } else {        
        if ($nombre_img == !NULL) {
            array_push($resultado, 0, "La imagen es demasiado grande");
                return $resultado;
        }
    }
}

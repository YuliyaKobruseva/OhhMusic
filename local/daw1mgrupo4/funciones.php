...
function selectConcierto($inicio, $cantidad) {
    $c = conectar();
    $select = "select * from (( concierto inner join local on concierto.local = local.usuLocal) inner join musico on concierto.musico = musico.usuMusico) where concierto.estado = 'Aceptado' and concierto.fecha>=curdate() order by concierto.fecha asc limit $inicio, $cantidad";
...
inner join fan on fanvotamusico.fan = fan.usuFan) group by musico.nombreArtistico limit 10";
    $resultado = mysqli_query($conexion, $select);
    desconectar($conexion);
    return $resultado;
}
